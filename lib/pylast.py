...
class SessionKeyGenerator(object):
    ...
    def get_web_flow(self, callback=None):
	url = '%(homepage)s/api/auth/?api_key=%(api)s' % \
	{"homepage": self.network.homepage, "api": self.network.api_key}
    ...
    def get_web_auth_url(self,callback=None):
		"""The user must open this page, and you first, then call
		get_web_auth_session_key(url) after that."""
    ...
    def get_web_auth_session_key_verbose(self, token):
